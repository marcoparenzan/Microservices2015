...
{
    public class QueryingOrderRepository : AggregateRootRepository<QueryingOrder, Guid>
    {
        protected override async Task OnSetAsync(QueryingOrder aggregateRoot)
        {
        ...
        protected override async Task<QueryingOrder> OnGetByIdAsync(Guid id)
        {
            var blobContainer = BlobClient.GetContainerReference("orders");
            var blobList = blobContainer.ListBlobs("order/" + id.ToString());
            if (!blobList.Any())
            {
                return new QueryingOrder { };
            }
